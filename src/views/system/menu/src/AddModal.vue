<template>
  <Modal @register="register" @ok="handleValidateButtonClick" @close="close" style="width: 50%">
    <div>
      <n-form
        ref="formRef"
        :model="model"
        ...
        <n-grid :cols="24" :x-gap="24">
          <n-form-item-gi :span="24" label="节点类型" path="type">
            <n-radio-group v-model:value="model.type" name="type">
              <n-space>
                <n-radio v-for="song in optionType" :key="song.value" :value="song.value">
                  {{ song.label }}
                </n-radio>
              </n-space>
            </n-radio-group>
          </n-form-item-gi>
          <n-form-item-gi :span="24" label="上级节点" path="parentId" v-if="model.type !== 1">
            <NTreeSelect
          ...
          <template v-if="model.type !== 3">
            ...
            <n-form-item-gi :span="24" label="路由地址" path="path">
              <NInput v-model:value="model.path" placeholder="请输入路由地址 如 /test" />
            </n-form-item-gi>
            <!-- type:3 权限标识  -->
            <n-form-item-gi :span="24" label="路由重定向" path="redirect">
              <NInput v-model:value="model.redirect" placeholder="router.redirect" />
            </n-form-item-gi>
            <n-form-item-gi
              :span="24"
              label="内嵌地址"
            ...
            >
              ...
              <NInput v-model:value="model.iframeSrc" placeholder="请输入外链地址" />
            </n-form-item-gi>
            <!-- 权限名称 -->
            ...
            <n-form-item-gi :span="6" label="路由缓存" path="keepAlive">
              <n-radio-group v-model:value="model.keepAlive" name="keepAlive" disabled>
                <n-space>
          ...
          <n-form-item-gi :span="6" label="排序" path="orderNo">
            <NInputNumber v-model:value="model.orderNo" placeholder="排序号" :min="0" :max="9999" />
          </n-form-item-gi>
          <n-form-item-gi :span="6" label="是否显示" path="state">
            <n-switch v-model:value="model.state" :checked-value="1" :unchecked-value="0" />
          </n-form-item-gi>
        </n-grid>
      </n-form>
    </div>
...
const emit = defineEmits(['update:showModal', 'refresh', 'update:type', 'update:parentId'])
