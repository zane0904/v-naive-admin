...
const useTable = function (props: BasicTableProps) {
  ...
  const setLoading = (value: boolean) => (unref(propsValue)!.dataTableProps!.loading = value)
  //   设置所有配置
  const setTableProps = (config: BasicTableProps) => unref(desc)!.setTableProps(config)
