...
export default defineComponent({
  ...
  setup(props, { attrs, slots, emit }) {
    ...
    const immediate = async () => {
      ...
      const { pageSizeField, pageIndexField, listField, pageTotalField } = pageSetting!
      try {
        let params = {
          ...(additionalParams ?? {}),
        ...
        if (!data) {
          console.warn(`获取接口数据为 ${typeof data} 类型,请检查 listField 字段是否正确`)
        } else if (afterFetch) {
    ...
    onMounted(() => {
      ...
      if (propsValue.value?.immediate) {
        if (!propsValue.value?.api) {
          console.warn('If immediate is enabled, please configure Api correctly!')
        } else {
          immediate()
        }
      }
    ...
    const mapSlots = {}
    Object.keys(slots).forEach(item => Reflect.set(mapSlots, item, slots[item]))
