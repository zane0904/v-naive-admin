...
export function generateNaiveDts(options: Options): Plugin {
  ...
  const pathResolve = (...args: string[]) => path.resolve(process.cwd(), ...args)

  const getAllComponentsNameFromNaiveDts = async (str: string) => {
    const componentsNames: string[] = []

    const componentsDtsPaths = Array.from(str.matchAll(/'(.+?)';/gi)).map(items => {
      return pathResolve('./node_modules/naive-ui/es/', items?.[1] ?? '', './index.d.ts')
    })
    ...
    await Promise.allSettled(componentsDtsPromises).then(results => {
      results.map(res => {
        ...
        const names: string[] = Array.from(
          res.value.matchAll(/export\s*\{\s*[\w_$\s,]*(?<!N)\s+(N[\w_$]+)[\w_$\s,]*\s*\}/g)
        ).map(items => {
          let name = items[1]
          if (!name) {
            return ''
  ...
  return {
    ...
    async configResolved() {
      const naiveComponentsPath = pathResolve('./node_modules/naive-ui/es/components.d.ts')
      ...
      componentsNames = componentsNames.filter(name => !excludeNames.includes(name))
